...
class Game:
    def __init__(self):
        super().__init__()
        self.number_map = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8,
                           "nine": 9}

    def handle(self, input: str) -> int:
        sum = 0
        for line in input.splitlines():
            sum += self.handle_line(line.strip())

        print(f"The total is {sum}")

    def handle_line(self, line, max_value ={"red": 12, "green" : 13, "blue": 14}):
        game_info = line.split(":")
        game = game_info[0].replace("Game", "")
        game_sets = game_info[1].strip().split(";")
        game_set_item_count = {"red": 0, "green": 0, "blue": 0}
        # print(line)
        for game_set in game_sets:
            for game_part in game_set.strip().split(","):
                part_normalized = game_part.strip()
                item_count = re.findall('\d+', part_normalized)[0]
                color_name = part_normalized.split(" ")[1]
                # print(f"{color_name}={item_count}")
                game_set_item_count[color_name]= max(int(item_count), game_set_item_count[color_name])

        power = 0
        for color, value in game_set_item_count.items():
            if power == 0:
                power = value
            else:
                power = power * value

            # if value > max_value[color]:
            #     return 0
        print(f"power {power}")
        return int(power)
...
if __name__ == "__main__":
    Game().handle("""Game 1: 3 blue, 2 green, 6 red; 17 green, 4 red, 8 blue; 2 red, 1 green, 10 blue; 1 blue, 5 green
